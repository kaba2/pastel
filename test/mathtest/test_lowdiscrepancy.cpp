...
namespace
{
	...
	void testHalton()
	{
		...
		{
			const Point2 point = haltonSequence<real>(i, Point<integer, 2>(3, 2));

			color = linear(aColor, bColor, (real)i / 100000);

			drawPixel(asPoint(asVector(point) * Vector<real, 2>(Width, Height)), color, arrayView(image));
		}
	...
	void testHammersley()
	{
		...
		for (integer i = 0;i < 100;++i)
		{
			const Point2 point = hammersleySequence<real>(i, Point<integer, 1>(2), 100000);
