...
namespace
{
	...
	void test()
	{
		REPORT(std::abs(relativeError<real>(2, 1) - 1) > 0.001);
		REPORT(std::abs(relativeError<real>(3, 1) - 2) > 0.001);
		REPORT(std::abs(relativeError<real>(4, 1) - 3) > 0.001);
		REPORT(std::abs(relativeError<real>(5, 1) - 4) > 0.001);
		REPORT(std::abs(relativeError<real>(4, 2) - 1) > 0.001);
		REPORT(std::abs(relativeError<real>(6, 2) - 2) > 0.001);
		REPORT(std::abs(relativeError<real>(8, 2) - 3) > 0.001);
	...
	void addTests()
	{
		sysTestList().add("math_functions", test);
	}
